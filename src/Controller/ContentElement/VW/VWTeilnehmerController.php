...
*/
        ...
        $my_script_txt = <<< EOT
          ...
          function teilnehmerLoeschen(obj) {
            ...
            jQuery.get(url, function(data, status){            
                jQuery("#eingabe").html("&#x23F1; 5 sec<br>" + data['data']);
                // 8 Sekunden warten, dann reload
                setTimeout(function() {
                    location.reload();
                }, 5000);  // 8000 ms = 8 Sekunden
            });
          }
...
EOT;
        ...
        $html.=$c->thead();
          $html.=$c->tr();
            $html.=$c->th("Bez, Er, SZ, AF, VF, HF, FI").$c->th("ID").$c->th("Name").$c->th("Kurzname").$c->th("Email").$c->th("Passwort");
          $html.=$c->end_tr();
        $html.=$c->end_thead();
        $html.=$c->tbody();
        ...
        foreach ($this->Teilnehmer as $k=>$tln) {
          ...
          $html.=$c->end_td();
          $html.=$c->td((string) $tln['ID']).$c->td($tln['Name']).$c->td($tln['Kurzname']).$c->td($tln['Email']).$c->td($tln['pwd'] ?? '');
	      $html.=$c->end_tr() . "\n";
	      $html.=$c->tr().$c->td(array("colspan"=>"6"));
